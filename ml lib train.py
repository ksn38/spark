...
spark.read.csv("/user/305_kozik/titanic/titanic.csv").createOrReplaceTempView("titanic")
spark.read.csv("/user/305_kozik/titanic/titanic_test.csv").createOrReplaceTempView("titanic_test")

spark.sql("select * from titanic").show(10, False)
spark.table('titanic').select('*').show()
